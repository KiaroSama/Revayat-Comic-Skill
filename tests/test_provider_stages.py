"""The stages that USE a provider: visual QA, the image editor, the optional
machine translator, and what each of them does when the provider misbehaves.

Split out of `test_providers.py`, which had grown past the size a file stays
readable at. That file keeps the provider boundary itself — registration, the
return contract, and the guarantee that nothing a provider returns reaches a
pixel it was not authorised for.
"""
...
from test_providers import _needs_the_provider  # noqa: F401
...
def test_building_a_context_over_an_unmerged_page_is_refused(detected):
    ...
    (root / "worksheets" / f"{first}.done.txt").write_text(
        ...
        encoding="utf-8")
    assert context.unmerged_before(detected, ir.load_doc(detected), second)[0] == [first]
...
def test_the_lock_lifts_once_the_page_is_merged(detected):
    ...
    _translate_page(detected, first, {})
    assert context.unmerged_before(detected, ir.load_doc(detected), second)[0] == []
