...
.EXAMPLE
    .\install.ps1
    .\install.ps1 -Agent claude -Scope project -Path C:\work\my-comic
#>
[CmdletBinding()]
param(
...
Write-Host ''
Write-Host "Revayat Comic installer" -ForegroundColor Cyan
...
$BeginMark = '<!-- BEGIN revayat-comic -->'
$EndMark   = '<!-- END revayat-comic -->'
...
function Write-AgentsPointer {
    ...
    $section = @(
        ...
        '',
        'To translate a comic chapter into Persian and produce a CBZ or PDF, follow',
...
Write-Host "  python `"$(Join-Path $Source 'scripts\revayat-comic.py')`" doctor"
Write-Host ''
Write-Host 'Activate the skill by its name, which is "revayat-comic" — the value of'
