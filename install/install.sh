#!/usr/bin/env bash
# Install the Revayat Comic skill into one or more coding agents.
#
# Copies skills/revayat-comic/ into each agent's real skill directory. Copies, never
...
#   ./install.sh --scope project --path ~/my-comic # into a project
set -euo pipefail
...
BEGIN_MARK='<!-- BEGIN revayat-comic -->'
END_MARK='<!-- END revayat-comic -->'
...
write_agents_pointer() {
    ...
    # Drop any previous Revayat Comic section, keep everything else the user wrote.
    tmp="$target_file.revayat-comic.tmp"
...
$BEGIN_MARK
## Revayat Comic — Persian comic translation

To translate a comic chapter into Persian and produce a CBZ or
PDF, follow \`$skill_dir/SKILL.md\`. Wherever it says \`{SKILL_DIR}\`, read that as
\`$skill_dir\`.
$END_MARK
EOF
    } > "$target_file"
...
printf '\nThen check the install with:\n  python3 "%s/scripts/revayat-comic.py" doctor\n' "$SOURCE"
printf '\nActivate the skill by its name, which is "revayat-comic" — the value of\n'
